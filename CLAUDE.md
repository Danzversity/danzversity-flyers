...
- **🕒 Schedule for later (v1.12.0)** — every post path (composed / finished-flyer / repost) can be queued instead of sent: tick "Schedule for later", pick a local date+time, and the **same preview → confirm** gate runs first. The job lives on the **rail (v6)**, which holds the prepared JPEG in KV and fires it from a **Cloudflare cron (`*/5`)** — it goes out whether or not this page, the laptop, or the app is open. Queue panel lists what's waiting (cancellable) plus what recently fired. **Never** put a timed send on the app-side scheduler.
- **🔁 Repost (v1.11.0)** — browse the saved `FLYERS/{event}/{YYYY-MM}/{bucket}` tree in the tool and post something again **straight from Drive**. The browser only ever sees a proxied thumbnail; at post time the SERVER pulls the asset from Drive and hands it to the rail — no download, no re-upload, no re-compose (and no Remove.bg call). Placement per asset comes from its real pixel dimensions. `fit` is skipped when the source already matches the target, so a repost of our own 9:16 goes out byte-exact.
...
- **Scheduling:** `POST /schedule-social` (same body as `/post-social` + `when`) · `GET /scheduled` · `POST /unschedule` — all proxy the rail's v6 endpoints. `prepareSocialJpeg()` is shared with `/post-social` on purpose: a post and the same post scheduled must be byte-identical, so there is only one preparation path.
- **Repost:** `GET /drive-browse?folderId=` (folders + images with real w/h from `imageMediaMetadata`; library pools hidden and loose files suppressed at the root) · `GET /drive-thumb?id=` (proxied so the browser needs no Drive credentials)
- **`POST /post-social`** — base64 **or `driveFileId`** + caption + platforms + placement + `mode` (`preview`|`send`) → the danzversity-social rail. Optional **`fit`** (a `brand.sizes` key, `fitPolicy` `letterbox`|`smart`) re-frames the image first — the "post a finished flyer" path uses it to make any ratio Meta-legal; composed flyers pass no `fit` and go out exactly as rendered. Un-fitted uploads over 1920px are downscaled (Meta caps feed images at 8MB).
...
- **Chassis geometry rules (v1.5.2):** bottom-block HORIZONTAL dims are fractions of WIDTH (H-fractions balloon 42% on 9:16 and ran the CTA under the QR — the 8/9 Flo Master overlap); person cutouts are `.trim()`ed to the subject's alpha bbox before zone-fit (Remove.bg returns the full source canvas — untrimmed, a subject renders at ~12%); `qrGeom()` is the single source for QR placement; and `qaGate()` runs on EVERY compose — QR/bottom-block intersection or an alpha-weighted subject share <2% throws and rejects the flyer. Never bypass or soften the gate; fix the layout instead.
